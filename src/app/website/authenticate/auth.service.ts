...
export class AuthService {
  ...
  private _refreshingTokenInProgressSubject = new BehaviorSubject<
    string | undefined
  >(undefined);
  readonly refrereshingTokenInProgress$ =
    this._refreshingTokenInProgressSubject.asObservable();
  ...
  readonly hasEmployeeSellerPermission =
    this._hasEmployeeSellerPermission.asReadonly();

  private readonly _hasEmployeeManagerPermission = signal(false);
  readonly hasEmployeeManagerPermission =
    this._hasEmployeeManagerPermission.asReadonly();

  private readonly _hasEmployeeAdminPermission = signal(false);
  readonly hasEmployeeAdminPermission =
    this._hasEmployeeAdminPermission.asReadonly();

  private readonly _hasEmployeeSuperAdminPermission = signal(false);
  readonly hasEmployeeSuperAdminPermission =
    this._hasEmployeeSuperAdminPermission.asReadonly();
  ...
  get authTokenState() {
    ...
    const expiryAccessTokenDate =
      this._tokenStorageService.expiryAccessTokenDate;
    const expiryRefreshTokenDate =
      this._tokenStorageService.expiryRefreshTokenDate;
  ...
  getValidatorParameters() {
    return this._client
      .get<
        ApiResponse<AuthValidatorParameters>
  ...
  getUserMe() {
    return this._client
      .get<ApiResponse<User>>(`${this._baseUrl}/users/me`)
      .pipe(
        map((response) => {
  ...
  signIn(model: SignInAuth) {
    return this._client
      ...
      .pipe(
        tap((response) => this._tokenStorageService.saveData(response.data)),
      );
  }

  signUpGuest() {
    return this._client
      .post<ApiResponse<Auth>>(`${this._baseUrl}/guests/sign-up`, {})
      .pipe(
        tap((response) => {
          this.setGuestRole();
      ...
      switchMap(() =>
        this._client
          ...
          .pipe(
            tap((response) => {
              this._tokenStorageService.saveData(response.data);
              this._refreshingTokenInProgressSubject.next(
